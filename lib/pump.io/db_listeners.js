    ...
    'core.dbPubSub': {
      'dbPubSub': function(session_id, channel, data) {
        ...
        if (state === 'unavailable') {
          return this.db.srem(this.rkey('session_id', session_id, 'channels'), channel, __bind(function(err, res) {
            return this.db.srem(this.rkey('channel', channel, 'session_ids'), session_id, __bind(function(err, res) {
              return this.db.get(this.rkey('channel', channel, 'session_id', session_id, 'data'), __bind(function(err, current_data) {
                current_data = this.parseJSON(current_data);
                presence = current_data.state === 'subscribed' ? false : true;
                return this.db.del(this.rkey('channel', channel, 'session_id', session_id, 'data'), __bind(function(err, res) {
                  if (presence) {
          ...
          return this.db.sadd(this.rkey('session_id', session_id, 'channels'), channel, __bind(function(err, res) {
            return this.db.sadd(this.rkey('channel', channel, 'session_ids'), session_id, __bind(function(err, res) {
              return this.db.set(this.rkey('channel', channel, 'session_id', session_id, 'data'), data_s, __bind(function(err, res) {
                if (presence) {
                  return this.db.smembers(this.rkey('channel', channel, 'session_ids'), __bind(function(err, session_ids) {
                    ...
                    return this.db.mget(data_keys, __bind(function(err, datas) {
                      return this.db.mget(user_id_keys, __bind(function(err, user_ids) {
                        var data_i, i, sid, _len, _results;
                        this.s2s({
                          type: 'presence',
                        ...
                        for (i = 0, _len = session_ids.length; i < _len; i++) {
                          sid = session_ids[i];
                          _results.push(sid !== session_id ? (data_i = datas[i], data_i ? (data_i = this.parseJSON(data_i), data_i.state !== 'subscribed' ? this.s2s({
                            type: 'presence',
      ...
      'dbPubSubLeaveAll': function(session_id) {
        ...
        return this.db.smembers(this.rkey('session_id', session_id, 'channels'), __bind(function(err, channels) {
          ...
          for (_i = 0, _len = channels.length; _i < _len; _i++) {
            channel = channels[_i];
            _results.push(this.emit('dbPubSub', session_id, channel, {
              state: 'unavailable'
            }));
          }
          return _results;
