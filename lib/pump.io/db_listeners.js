  ...
  module.exports = {
    ...
    'core.dbAuthenticated': {
      'dbAuthenticated': function(session_id, user_id) {
        ...
        this.db.set(this.rkey('session_id', session_id, 'user_id'), user_id, __bind(function(err, res) {
          this.db.sadd(this.rkey('user_id', user_id, 'session_ids'), session_id, __bind(function(err, res) {
            this.emit('dbPubSubRejoinAll', session_id);
          }, this));
        }, this));
      }
    },
    ...
    'core.dbServerSweep': {
      'dbServerSweep': function() {
        ...
        this.log("core.dbServerSweep by " + this.server_id);
    ...
    'core.dbPubSub': {
      'dbPubSub': function(session_id, channel, data, rid) {
                  ...
                  this.db.smembers(this.rkey('channel', channel, 'session_ids'), __bind(function(err, session_ids) {
                    ...
                    this.db.mget(data_keys, __bind(function(err, datas) {
                      this.db.mget(user_id_keys, __bind(function(err, user_ids) {
                        var data_i, from_me, i, session_ids_except_me, sid, _len;
                        session_ids_except_me = (function() {
                          var _i, _len, _results;
                          _results = [];
                          for (_i = 0, _len = session_ids.length; _i < _len; _i++) {
                            sid = session_ids[_i];
                        ...
                        from_me = this.resource(session_id, user_ids[session_ids.indexOf(session_id)]);
                        this.s2s({
                          type: 'presence',
                          rid: rid,
                          from: from_me,
                        ...
                        this.s2s({
                          type: 'presence',
                          from: from_me,
                          session_ids: session_ids_except_me,
                          channel: channel,
                          data: data
                        });
                        if (performed === 1) {
                          for (i = 0, _len = session_ids_except_me.length; i < _len; i++) {
                            sid = session_ids_except_me[i];
                            data_i = datas[i];
                            if (data_i) {
    ...
    'core.dbPubSubRejoinAll': {
      'dbPubSubRejoinAll': function(session_id) {
        this.log("core.dbPubSubRejoinAll: " + session_id);
        this.db.smembers(this.rkey('session_id', session_id, 'channels'), __bind(function(err, channels) {
          var channel, data_keys;
          data_keys = (function() {
            var _i, _len, _results;
            _results = [];
            for (_i = 0, _len = channels.length; _i < _len; _i++) {
              channel = channels[_i];
              _results.push(this.rkey('channel', channel, 'session_id', session_id, 'data'));
            }
            return _results;
          }).call(this);
          this.db.mget(data_keys, __bind(function(err, datas) {
            var channel, data_i, i, _len, _results;
            _results = [];
            for (i = 0, _len = channels.length; i < _len; i++) {
              channel = channels[i];
              data_i = datas[i];
              _results.push(data_i ? (data_i = this.parseJSON(data_i), data_i.state || (data_i.state = 'available'), this.emit('dbPubSub', session_id, channel, data_i)) : void 0);
            }
            return _results;
          }, this));
        }, this));
