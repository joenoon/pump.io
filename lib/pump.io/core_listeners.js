  ...
  module.exports = {
    ...
    'core.authenticationSuccess': {
      'authenticationSuccess': function(client, user_id) {
        this.log("core.authenticationSuccess: " + client.sessionId + ", " + user_id);
    ...
    'core.authenticationFailed': {
      'authenticationFailed': function(client) {
        this.log("core.authenticationSuccess: " + client.sessionId);
        client.send({
          type: 'authenticated',
          data: {
            result: false
      ...
      'payload': function(payload) {
        ...
        if ((_ref = this.server_id, __indexOf.call(payload.server_ids, _ref) >= 0) && ((_ref2 = payload.type) === 'ping' || _ref2 === 'pong')) {
          if (payload.type === 'ping') {
            this.log("ping on " + this.server_id);
            this.s2s({
              server_ids: [payload.origin_server_id],
              type: 'pong'
            });
          } else if (payload.type === 'pong') {
            this.log("pong on " + this.server_id + " from " + payload.origin_server_id);
    ...
    'core.pubsub': {
      'pubsub': function(client, channel, data) {
        this.emit('dbPubSub', client.sessionId, channel, data);
      }
    }
  };
}).call(this);
