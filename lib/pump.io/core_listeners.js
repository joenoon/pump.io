  ...
  module.exports = {
      ...
      'payload': function(payload) {
        ...
        if ((_ref = this.server_id, __indexOf.call(payload.server_ids, _ref) >= 0) && ((_ref2 = payload.type) === 'ping' || _ref2 === 'pong')) {
          if (payload.type === 'ping') {
            console.log("ping on " + this.server_id);
            return this.s2s({
              server_ids: [payload.origin_server_id],
              type: 'pong'
            });
          } else if (payload.type === 'pong') {
            console.log("pong on " + this.server_id + " from " + payload.origin_server_id);
    ...
    'core.payload.message': {
      'payload': function(payload) {
        ...
        return payload.session_ids.forEach(__bind(function(session_id) {
          if (session_id in ci) {
            return ci[session_id].send(client_payload);
          }
        }, this));
      }
    },
    ...
    'core.pubsub': {
      'pubsub': function(client, channel, data) {
        return this.emit('dbPubSub', client.sessionId, channel, data);
      }
    }
  };
}).call(this);
