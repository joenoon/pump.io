(function() {
  var EventEmitter, Pump, clientVersion, express, fs, io, redis, sys, url;
  ...
  Pump = (function() {
    ...
    function Pump(options) {
      ...
      this.server_sweeper = options.server_sweeper || false;
      ...
      this.server.use(__bind(function() {
        return this._checkRequest.apply(this, arguments);
      }, this));
      ...
      this.subscriber.subscribe(this.cluster_name);
      this.__bind_listeners();
      if (this.server_sweeper) {
    ...
    Pump.prototype.use = function(unique_name, type, fn) {
      ...
      return this.on(type, this.__listeners[unique_name]);
    };
    Pump.prototype.useAll = function(hash) {
      var data, fn, type, unique_name, _results;
      _results = [];
      for (unique_name in hash) {
        ...
        _results.push((function() {
          ...
          for (type in data) {
            fn = data[type];
            _results.push(this.use(unique_name, type, fn));
          }
          return _results;
        }).call(this));
      }
    ...
    Pump.prototype.unuse = function(unique_name, type) {
      ...
      return delete this.__listeners[unique_name];
    };
    ...
    Pump.prototype.internalChannel = function(channel) {
      return "" + this.cluster_name + channel;
    };
    Pump.prototype.s2s = function(obj) {
      var session_keys, user_id;
      ...
      if (obj.user_ids.length > 0) {
        ...
        return this.db.smembers(this.rkey('channel', obj.channel, 'session_ids'), __bind(function(err, session_ids) {
        ...
        return this.publisher.publish(this.cluster_name, this.toJSON(obj));
      } else {
        return console.log("s2s not sent because no recipients: " + (sys.inspect(obj)));
      }
    };
    ...
    Pump.prototype.graceful_exit = function() {
      console.log('graceful shutdown...');
      process.nextTick(__bind(function() {
        if (this.__server_sweeper_interval) {
          clearInterval(this.__server_sweeper_interval);
        }
        this.server.close();
        this.emit('disconnectAll');
        this.subscriber.shutting_down = true;
        this.subscriber.unsubscribe(this.cluster_name);
    ...
    Pump.prototype.__bind_listeners = function() {
      ...
      return this.subscriber.on('unsubscribe', __bind(function(channel, subscription_count) {
    ...
    Pump.create = function(cluster_name, port, host, server_sweeper) {
